...
| `cleaned_airbnb_final.csv` | Cleaned dataset (102,301 rows × 24 columns) — ready for analysis |
...
- **Parsed** `price` & `service_fee` from strings (`"$966"`) → float
- **Fixed typos** in borough names (`brookln` → `Brooklyn`, `manhatan` → `Manhattan`)
...
- **Converted** `construction_year` from float → int, parsed `last_review` as datetime
...
| 3 | **Price ≠ Quality** | Near-zero correlation (−0.005) between price and review ratings |
...
| 8 | **Top neighbourhoods cluster** in Manhattan & Brooklyn | Bedford-Stuyvesant, Williamsburg, Harlem, Hell's Kitchen |
...
| **Matplotlib & Seaborn** | EDA visualizations — histograms, bar charts, heatmaps |
...
**Size:** 102,599 rows × 26 columns (raw) → 102,301 rows × 24 columns (cleaned)  
